...
func TestCronsList(t *testing.T) {
	...
	scenarios := []tests.ApiScenario{
		...
		},
		{
			...
			ExpectedStatus: 200,
			ExpectedContent: []string{
				`{"id":"__pbLogsCleanup__","expression":"0 */6 * * *","paused":false}`,
				`{"id":"__pbDBOptimize__","expression":"0 0 * * *","paused":false}`,
				`{"id":"__pbMFACleanup__","expression":"0 * * * *","paused":false}`,
				`{"id":"__pbOTPCleanup__","expression":"0 * * * *","paused":false}`,
			},
			ExpectedEvents: map[string]int{"*": 0},
		},
	}
