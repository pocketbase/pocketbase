...
type plugin struct {
	...
	initialDiscovery bool // true only during initial discovery phase
...
func (p *plugin) onBootstrap(e *core.BootstrapEvent) error {
	...
	// Set default node type if empty
	if settings.Sync.NodeType == "" {
		settings.Sync.NodeType = "add"
		if err := p.app.Save(settings); err != nil {
			log.Printf("Warning: Failed to save default node type: %v", err)
		}
	}
...
func (p *plugin) startSync(settings ...*core.Settings) error {
	...
	nodeType := syncSettings.Sync.NodeType
	if nodeType == "initial" {
		// Initial node - don't do discovery, just set up for publishing
		log.Printf("Initial node - skipping discovery, setting up for publishing")
		go func() {
			...
			if hasSnapshot {
				log.Printf("Existing snapshot found in NATS JetStream")
			} else {
				log.Printf("No existing snapshot found - creating initial snapshot for initial node")
				// Create self-record BEFORE snapshot so it's included
				if err := p.ensureAndPublishSelfRecord(); err != nil {
					log.Printf("Warning: Failed to create self record before snapshot: %v", err)
				}

				if _, err := p.snapshotManager.CreateSnapshot(); err != nil {
					log.Printf("Warning: Failed to create initial snapshot: %v", err)
				} else {
					log.Printf("Initial snapshot created for initial node")
				}
			}

			// Mark initial node as ready to publish
			p.mu.Lock()
			p.syncing = false
			p.mu.Unlock()
			log.Printf("Initial node ready - beginning to publish schema and record changes")
		}()
	} else {
		// Add node - check for existing instances and do discovery
		...
		if len(allInstances) > 1 {
			...
			// No existing instances in _pbSync - this could be a new sync setup or restart
			...
			go func() {
				...
				if hasSnapshot {
					...
					// IMPORTANT: Create self-record BEFORE snapshot so it's included
					// This ensures other instances can discover this instance through the snapshot
					if err := p.ensureAndPublishSelfRecord(); err != nil {
						log.Printf("Warning: Failed to create self record before snapshot: %v", err)
					}
...
func (p *plugin) onSettingsUpdate(e *core.SettingsUpdateRequestEvent) error {
	...
	if !oldSettings.Sync.Enabled && newSettings.Sync.Enabled {
		...
		if newSettings.Sync.NodeType == "" {
			log.Printf("Cannot start sync: nodeType is required")
		...
		// Validate additional fields for "add" node type
		if newSettings.Sync.NodeType == "add" {
			if newSettings.Sync.TargetInstanceAddress == "" {
				log.Printf("Cannot start sync: targetInstanceAddress is required for add node")
				// Don't fail the settings update, but don't start sync either
				return e.Next()
			}
			if newSettings.Sync.TargetInstanceID == "" {
				log.Printf("Cannot start sync: targetInstanceID is required for add node")
				// Don't fail the settings update, but don't start sync either
				return e.Next()
			}
		}
		...
		log.Printf("Sync enabled as %s node, starting NATS server...", newSettings.Sync.NodeType)
		if err := p.startSync(newSettings); err != nil {
			log.Printf("Error starting sync: %v", err)
			// Don't fail the settings update, but log the error
		}

		// If this is an "add" node, trigger discovery for the target instance
		if newSettings.Sync.NodeType == "add" && newSettings.Sync.TargetInstanceAddress != "" && newSettings.Sync.TargetInstanceID != "" {
			go func() {
				// Wait a moment for sync to be fully initialized
				time.Sleep(2 * time.Second)
				log.Printf("Triggering discovery for target instance %s at %s", newSettings.Sync.TargetInstanceID, newSettings.Sync.TargetInstanceAddress)
				if err := p.discoverInstance(newSettings.Sync.TargetInstanceID, newSettings.Sync.TargetInstanceAddress); err != nil {
					log.Printf("Warning: discovery failed for target instance %s: %v", newSettings.Sync.TargetInstanceID, err)
					return
				}
	...
	if newSettings.Sync.Enabled {
		...
		if needsRestart {
			...
			if newSettings.Sync.NodeType == "" {
				log.Printf("Cannot restart sync: nodeType is required")
			...
			if newSettings.Sync.NodeType == "add" {
				if newSettings.Sync.TargetInstanceAddress == "" {
					log.Printf("Cannot restart sync: targetInstanceAddress is required for add node")
					// Don't fail the settings update, but don't restart sync either
					return e.Next()
				}
				if newSettings.Sync.TargetInstanceID == "" {
					log.Printf("Cannot restart sync: targetInstanceID is required for add node")
					// Don't fail the settings update, but don't restart sync either
					return e.Next()
				}
			}
...
func (p *plugin) processEvents() {
	for {
		...
		case event := <-p.subscriber.Events():
			...
			// CRITICAL: After processing REMOTE events, republish them to our own stream
			// This ensures other instances can consume events that were originally published elsewhere
			// We only republish remote events, not local or already republished events to prevent loops
			// IMPORTANT: Do NOT republish during discovery mode to prevent infinite loops
			p.mu.Lock()
			inDiscoveryMode := p.syncing
			p.mu.Unlock()

			if err == nil && event.Source == events.EventSourceRemote && !inDiscoveryMode {
				republishedEvent := events.NewRepublishedEvent(event)
				p.publisher.PublishAsync(republishedEvent)
				log.Printf("Republished remote event from instance %s to local stream: %s/%s", event.InstanceID, event.Collection, event.RecordID)
			} else if err == nil && event.Source == events.EventSourceRemote && inDiscoveryMode {
				log.Printf("Skipping republication of remote event during discovery mode: %s/%s", event.Collection, event.RecordID)
			}
		...
		if err == nil {
			...
			if err == nil && snap.InstanceID != p.instanceID {
				...
				if err == nil {
					...
					// During initial discovery (p.syncing=true), we want to apply snapshots even if they're empty
					// This ensures schema synchronization and resolves conflicts with default PocketBase collections
					p.mu.RLock()
					inInitialDiscovery := p.syncing
					p.mu.RUnlock()

					if hasNonSystemCollections && len(snap.Collections) == 0 && totalRecordsInSnapshot == 0 && !inInitialDiscovery {
...
func (p *plugin) startDiscoveryCompletionFlow() {
	...
	go func() {
		...
		log.Printf("Starting iterative discovery process to find all instances...")
		// This ensures we discover ALL instances (like A) that were learned from the initially discovered instance (B)
		// and consume ALL their events (including schema deletions like the config collection)
		maxIterations := 10 // Prevent infinite loops
		for iteration := 0; iteration < maxIterations; iteration++ {
			log.Printf("=== Discovery iteration %d ===", iteration+1)
			log.Printf("Checking _pbSync records for instances we haven't connected to yet...")
		...
		// Set initialDiscovery to false - no more discovery phases
		p.mu.Lock()
		p.initialDiscovery = false
		p.mu.Unlock()
...
func (p *plugin) discoverAllInstancesFromPBSync() int {
	...
	// Debug: Log each record we find
	for _, record := range records {
		log.Printf("Found _pbSync record: instanceID=%s, natsAddress=%s",
			record.GetString("instanceID"),
			record.GetString("natsAddress"))
	}
	discoveredCount := 0
	for _, record := range records {
		...
		log.Printf("DEBUG: GetPendingCountForInstance(%s) returned %d", instID, pending)
		if pending >= 0 {
			// Already connected
